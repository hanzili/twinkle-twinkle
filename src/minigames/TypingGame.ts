...
export class TypingGame {
        ...
        try {
            if (!this.typingSound || !this.typingSound.isPlaying) {
                this.typingSound = this.scene.sound.add('type', {
                    loop: true,
                    volume: 10.0,
                    rate: 0.8  // slower playing speed
                });
                this.typingSound.play();
            }

            // Every time we press the key, reset the timer for "automatically stop playing the sound"
            if (this.typingStopTimer) {
                this.typingStopTimer.remove(false);
            }
            this.typingStopTimer = this.scene.time.delayedCall(1000, () => {
                if (this.typingSound && this.typingSound.isPlaying) {
    ...
    private async end(success: boolean): Promise<void> {
        ...
        // Restore glow hints if available (cast required to access recreateGlowHints)
        if ((this.scene as any).recreateGlowHints) {
            (this.scene as any).recreateGlowHints();
        }
